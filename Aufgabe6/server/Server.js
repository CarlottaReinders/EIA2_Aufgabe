...
(function (L05_Server) {
    ...
    let port = Number(process.env.PORT); // Die Variable "port" vom typ number wird erstellt
    if (!port) // Wenn Port nicht der Fall ist, wird die nächste Zeile ausgeführt
        port = 8100; // Port wird zu 8100
    let server = Http.createServer(); // Die Variable "server" vom Typ Http.Server wird erstellt und dieser Variable wird die Funktion createServer() zugewiesen
